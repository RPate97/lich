...
The dashboard at `http://lich.localhost:3300/` is a single view of every running lich stack on the machine.
...
The dashboard is what makes N-agents-in-N-worktrees observable. Without it:

- You forget which worktrees have running stacks.
...
The daemon (and therefore the dashboard) autostarts the first time any `lich` command needs it `lich up`, `lich dashboard`, `lich urls`, etc.
