...
**Goal:** Ship `levelzero test [unit|integration|e2e]` with auto-detected stack env injection. Define `TestRunnerAdapter` (Vitest impl, Playwright impl). Implement transactional-rollback integration test isolation via a small Prisma wrapper.
...
- `levelzero test` resolves stack context, derives `DATABASE_URL` + `API_URL` from the registry, injects them into the test runner's env, and chooses the adapter based on `[unit|integration|e2e]` subcommand:
  ...
  - `e2e` → playwright test, env injected; needs `levelzero dev` running.
...
tools/cli/src/
  ...
  commands/
    test.ts                     # levelzero test [unit|integration|e2e]
```
...
| 07.5 | `levelzero test` command (dispatches to adapter by subcommand) | 3 | `commands/test.ts` |
...
- `levelzero test unit` runs unit tests with no env injection.
- `levelzero test integration` runs integration tests against the current worktree's live Postgres (DATABASE_URL injected); `withRollback(prisma, fn)` keeps state clean between tests.
- `levelzero test e2e` runs Playwright tests against the current worktree's web service.
