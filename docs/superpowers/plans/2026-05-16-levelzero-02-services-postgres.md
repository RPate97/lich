# levelzero Plan 02 — Service contract + Postgres Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Introduce the `Service` interface, the port allocator, the worktree-namespaced Docker resource scheme, and Postgres as the first service implementation. Add `levelzero dev`, `stop`, `reset`, and `stacks stop --all` so a real stack can be brought up and torn down per worktree, in parallel with other worktrees, with no port or container-name collisions.
...
```
tools/cli/src/
  registry-lock.ts                 # advisory file lock for ~/.levelzero/registry.json
  services/
  ...
  commands/
    dev.ts                         # levelzero dev
    stop.ts                        # levelzero stop
    reset.ts                       # levelzero reset
    stacks/
      stop-all.ts                  # levelzero stacks stop --all
tools/cli/tests/
  (mirrored layout)
```
...
| 02.7 | `levelzero dev` (single-worktree) | 3 | 02.4, 02.6 |
| 02.8 | `levelzero stop` | 3 | 02.6, 02.7 |
| 02.9 | `levelzero reset` | 3 | 02.7, 02.8 |
| 02.10 | `levelzero stacks stop --all` | 4 | 02.8 |
...
- All container names, network names, and volume names go through the helpers in `src/docker/naming.ts` — no string-literal `levelzero-*-` anywhere else.
...
- Whether to use Docker Compose v2 (`docker compose ...`) or plain `docker run` for service lifecycle. **Decision:** Compose v2 — it gives us project-scoped lifecycle (`docker compose -p <project> down -v`) for free, and the worktree key becomes the compose project name. The per-stack `docker-compose.yml` is generated at `dev` time into `.levelzero/<key>/docker-compose.yml`.
...
- From a scratch dir with `levelzero.config.ts`:
  1. `levelzero dev` brings up Postgres in a container named `levelzero-<key>-postgres`, prints the allocated port and `DATABASE_URL`.
  2. `levelzero stacks current` reports `running: true` with the port and container.
  ...
  5. `levelzero stop` in worktree A tears it down cleanly; B keeps running.
  6. `levelzero stacks stop --all` from anywhere tears down both.
  7. `levelzero reset` from a fresh worktree (re-running step 1 first) wipes the volume and brings up an empty DB.
