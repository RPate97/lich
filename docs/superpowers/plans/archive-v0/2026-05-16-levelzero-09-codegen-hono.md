...
- `lich gen client` resolves stack context, finds the api project, calls `backendAdapter.extractRoutes`, pipes to `frontendAdapter.generateClient`, writes to `packages/api-client/src/index.ts`.
...
| 09.5 | `lich gen client` command | 3 | `commands/gen/client.ts` |
...
- `lich gen client` in a project with a Hono api writes `packages/api-client/src/index.ts` exposing typed functions.
- The web frontend imports those functions; tsc resolves the types correctly.
- `lich check` now reports route-coverage status (not just `skip`).
