...
Real projects outgrow the default stack. They add Redis for caching, a queue worker, Meilisearch for search, Mailpit for dev email, a Python service, ClickHouse for analytics. levelzero is designed so adding these is the same code path the built-in services already use — not a plugin system bolted on later.

### The Service contract

Every running thing in a levelzero stack — Postgres, api, web, and anything a project adds — is a `Service` satisfying the same interface:

```ts
interface Service {
  name: string;                       // unique within stack, used in logs and CLI
  kind: 'docker' | 'owned' | 'external';
  portNames: string[];                // levelzero allocates actual numbers
...
- **Custom commands.** Project plugins can register additional `levelzero <verb>` commands.
- **Custom `check` rules.** Project plugins can add conformance rules to `levelzero check` (e.g. "every Redis-using route has a cache invalidation test").
...
Skills are a directory, not a hardcoded list. CLAUDE.md is regenerated by a CLI command (`levelzero skills index`) by scanning `tools/skills/` and the user's plugin paths. Project-shipped skills appear in the agent's index next to built-in ones, with no special treatment.
