/**
 * README quickstart e2e — Plan 6 Task B3 (LEV-443).
 *
 * The README's `## Quickstart` section promises a user can clone the repo,
 * `cd examples/dogfood-stack`, run `lich up`, and hit the printed URLs in
 * a browser. This test is that promise's executable contract: if a future
 * change to lich (or the dogfood-stack, or the friendly-URL plumbing)
 * breaks the quickstart, this test fails and someone updates either the
 * README or the broken behavior.
 *
 * Pool: this file lives in the `fast` pool (default profile is dev:fast —
 * api + web on the host, no docker, no postgres). The quickstart's `lich
 * up` with no profile argument resolves to `dev:fast` (per the dogfood
 * stack's `default: true` flag on that profile), so the test only needs
 * the fast-pool prerequisites.
 *
 * Pool manifest: deliberately NOT added to `_pool-manifest.ts`. The
 * compose pool is for tests that need the `dev` profile / postgres /
 * after_up migrations; the quickstart's commands resolve to dev:fast and
 * don't touch any of that, so adding this file to the manifest would
 * mis-bucket it.
 *
 * Test shape:
 *   - Hard-coded command sequence with a leading link-back comment
 *     ("if you change these, update the README's `## Quickstart`"). The
 *     LEV-443 implementation note recommends this over regex-extracting
 *     from README.md because a fragile parser is worse than the explicit
 *     coupling.
 *   - Mirrors README §Quickstart commands semantically: tmpdir copy of
 *     `examples/dogfood-stack/` (the README's `git clone … && cd
 *     examples/dogfood-stack` equivalent), `lich up` (the README's `lich
 *     up`), `lich urls` (the README's "print the URLs") followed by HTTP
 *     200 probes of the URLs it lists (the README's "open the URL in a
 *     browser" check).
 *   - Asserts api `/health` returns 200 JSON with `db: "stub"` — proves
 *     the dev:fast default flip is intact end-to-end (catches the same
 *     drift `helpers/dbmode.ts` exists to surface).
 *   - Asserts the first printed URL serves a 200 response — the README
 *     tells the user to open it in a browser, so a 200 is the minimum
 *     contract the README is making.
 *   - `lich down` cleans up; the `afterEach` is the safety net but the
 *     in-body teardown shortens the cleanup hook window (mirrors
 *     `basic-up.test.ts` pattern).
 *
 * The `## Run two stacks in parallel` section of the README is NOT tested
 * programmatically here. The parallel-stacks sentinel
 * (`tests/e2e/parallel-stacks.test.ts`) already covers the load-bearing
 * isolation contract end-to-end (two stacks, distinct ports, both serve
 * HTTP 200, one-down-leaves-other-alive). Re-implementing it here would
 * duplicate the heavy two-stack choreography under a different name
 * without adding coverage. Manual verification of the README's parallel
 * demo: follow the README's commands in two shells; see that
 * `parallel-stacks.test.ts` exercises the same shape automatically on
 * every CI run.
 *
 * Cleanup contract (testing-standards §"Resource cleanup contract"):
 *   - In-body `lich down` is the primary teardown.
 *   - `afterEach` runs `lich down` + `lich nuke --yes` even on test-body
 *     failure (the safety net).
 *   - Tmpdir + LICH_HOME are removed in `afterEach`.
 *   - `lich nuke --yes` is required so the Plan-5 daemon (which holds
 *     proxy port 3300) dies between tests — mirrors `basic-up.test.ts`
 *     and `dashboard-stack-detail.test.ts`.
 */
...
// ---------------------------------------------------------------------------
// Build the binary up front. Mirror of `basic-up.test.ts`'s
// `beforeAll`: skip the rebuild if `dist/lich` already exists, otherwise
// run `bun run build` and fail loudly on a broken build (a broken build
// is a real bug, not a reason to skip).
// ---------------------------------------------------------------------------
