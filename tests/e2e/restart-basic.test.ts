...
function teardownFixture(fix: Fixture): void {
  ...
  // fast cleanup path; vitest's hookTimeout caps at 60s anyway, so the
  // old values could never actually fire — they just masked teardown
  // hangs as the wrong error. `lich nuke --yes` completes sub-200ms
  // even when killing a live daemon (SIGTERM → 5s grace → SIGKILL).
  try {
    runLich(["down"], {
      cwd: fix.stackPath,
