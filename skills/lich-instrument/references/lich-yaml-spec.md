...
Hook stderr is captured to `<LICH_HOME>/stacks/<id>/hooks/<phase>-<idx>.log` and surfaced inline on completion. Use this for debugging hooks that swallow errors with `|| true` — the inline tail prints regardless of exit code, and the full combined stdout+stderr lives in the per-hook log file (rolling ~1 MB cap). Path shape: `before_up-0.log`, `after_down-2.log`, etc. (phase + 0-based index within the composed entries array).
