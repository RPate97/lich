 ...
 *   - Portless    (current default; ships in `@lich/plugin-portless`)
 *   - ngrok       (`@ngrok/ngrok` SDK or `ngrok` CLI)
 *   - Cloudflare Tunnel (`cloudflared`)
 *   - Tailscale Funnel
 *   - localtunnel / serveo (lightweight alternatives)
 *   - LAN-only no-op (also shipped by `@lich/plugin-portless` as `noop`)
 *
 ...
 *
 * NOTE: this `types.ts` lived in `@lich/plugin-portless` until LEV-174;
 ...
 * `@lich/plugin-portless` re-exports the same names from here for
