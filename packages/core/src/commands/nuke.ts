 ...
 * `nuke` still needs raw `docker ps`/`docker rm -f` because it
...
export { makeNukeCommand as makeStacksStopAllCommand };

export function makeNukeCommand(getRegistry: () => Registry): Command {
  return {
    name: 'nuke',
    describe: 'Tear down every running lich stack on this machine',
    async run(ctx) {
