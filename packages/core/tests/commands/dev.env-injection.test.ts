...
function postgresPlugin(): {
  ...
  const composeService: { name: string; def: ComposeServiceDef } = {
    ...
    def: {
      ...
      container_name: 'levelzero-test-postgres',
...
describe('levelzero dev — Plan 16 env injection (LEV-182)', () => {
  ...
  it('same source key resolves host vs container differently in one dev invocation', async () => {
    ...
    // Owned side: host URL.
    const logPath = join(projectDir, '.levelzero', 'logs', 'web.jsonl');
    const lines = readFileSync(logPath, 'utf8')
      .split('\n')
      .filter(Boolean)
      .map((l) => JSON.parse(l) as { message: string });
  ...
  it('importAll bulk source flows into compose AND owned services', async () => {
    ...
    const composeSvc: ComposeServiceDef = {
      ...
      container_name: 'levelzero-test-redis',
    ...
    const logPath = join(projectDir, '.levelzero', 'logs', 'consumer.jsonl');
    const lines = readFileSync(logPath, 'utf8')
      .split('\n')
      .filter(Boolean)
      .map((l) => JSON.parse(l) as { message: string });
  ...
  it('writes a .env.<service> snapshot under .levelzero/state/<wt>/env/ for every service (LEV-183)', async () => {
    ...
    // `.levelzero/state/<worktreeKey>/env/`, and the file contents match the
    ...
    expect(composeFile).toMatch(/^# generated by levelzero dev on .+ — DO NOT EDIT/m);
