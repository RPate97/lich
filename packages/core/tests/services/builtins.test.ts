...
describe('getBuiltinServices', () => {
  it('does not include postgres (extracted to @lich/plugin-postgres in LEV-148)', () => {
    const list = getBuiltinServices();
    const pg = list.find((s) => s.name === 'postgres');
    expect(pg).toBeUndefined();
  });

  it('does not include web (extracted to @lich/plugin-next in LEV-154)', () => {
    const list = getBuiltinServices();
    const web = list.find((s) => s.name === 'web');
    expect(web).toBeUndefined();
  });

  it('does not include api (extracted to @lich/plugin-hono in LEV-187)', () => {
    const list = getBuiltinServices();
