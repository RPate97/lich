...
describe('bin: plan-13 adapter commands end-to-end', () => {
  describe('adapter list', () => {
    it('exits 0 and returns JSON listing every known adapter with its slot', () => {
      ...
      // The three built-in impls cover three of the eight declared AdapterSlot
      // values (orm, auth, browser). The remaining slots are now contributed
      // by extracted plugins and only appear when the plugin is declared in
      // `levelzero.config.ts`:
