 ...
 *
 * Acquisition: write metadata to a per-attempt tmpfile, then `link(2)` it to
 * the lock path. POSIX `link` fails with EEXIST if the destination exists —
 * that's the mutex. Doing it this way (rather than `open(wx)` then
 * `writeFile`) eliminates the window where a waiter could observe a
 * freshly-created-but-empty lockfile and reclaim it as junk, letting two
 * callers believe they hold the lock.
 *
 * Stale lockfiles (PID gone, or past staleAfterMs) are reclaimed by the next
 * waiter so a SIGKILL can't deadlock the system.
 */
...
export interface WithFileLockOptions {
  ...
  timeoutMs?: number;
  /** Locks held longer than this with a dead PID are reclaimed. Default 60s. */
  staleAfterMs?: number;
...
async function readLockMeta(lockPath: string): Promise<LockMeta | undefined> {
  ...
  // JSON is the current format; fall back to plain-PID for older lockfiles.
  try {
...
async function reclaimIfStale(
  ...
  if (meta === undefined) {
    // No readable metadata. The link(2) strategy makes this impossible for
    // an actively-held lock, but a human or crashed older lich could leave
    // a zero-byte file. Require observing emptiness twice across 100ms before
    // reclaiming, so we never steal because of an in-flight write.
...
/**
 * Write metadata to a tmpfile, then `link(2)` it to `lockPath`. EEXIST → false
 * (someone else holds it); success → we hold it, with our PID metadata
 * already in place (no observable empty state). The tmpfile is always unlinked.
 */
...
/**
 * Run `fn` while holding an exclusive lock on `lockPath`. Spins with backoff
 * until acquired or `timeoutMs` elapses. Lock is always released (try/finally)
 * even if `fn` throws.
 */
