...
/**
 * In-memory SPA asset source. Mirrors the disk-backed `uiDir` path but
 * reads from a manifest (typically `embedded-ui.generated.ts`) so the
 * daemon binary can ship without a sidecar `ui/dist/` directory.
 *
 * Production shape: `{ get: getEmbeddedAsset }` re-exported from the
 * generated module. Tests pass synthetic objects so they can probe the
 * SPA-fallback / unknown-path behavior without touching disk.
 */
export interface EmbeddedAssetSource {
  /**
   * Return the asset at `path` (relative to the SPA root, no leading
   * slash, e.g. `"index.html"` or `"assets/index-abc.js"`). Returns
   * undefined when no such asset exists in this source.
   */
  get(
    path: string,
...
export interface DashboardServerOpts {
   ...
   * the server consults {@link embeddedUi} (if provided); when neither
   * is set, the root path returns a placeholder HTML.
   *
   * `uiDir` wins over `embeddedUi` so a developer can override the
   * embedded bundle via `LICH_UI_DIR=./local/dist` without rebuilding
   * the daemon binary.
   */
  uiDir?: string;
  /**
   * Optional in-memory SPA assets, typically the embedded manifest
   * generated by `scripts/embed-ui.ts` and imported as
   * `embedded-ui.generated.ts`. Used only when {@link uiDir} is unset.
   *
   * Same SPA semantics as the disk path: exact-match wins; missing
   * non-API paths fall back to embedded `index.html` so React Router
   * can take over. When the source has no `index.html` (or no assets at
   * all), the handler falls through to the placeholder HTML.
   */
...
export async function startDashboardServer(
  ...
  const handler = async (req: Request): Promise<Response> => {
    ...
    // -- Embedded SPA (release-binary path) --
    // Same semantics as the disk branch: exact-match first, then
    // index.html fallback for React Router. Only consulted when there's
    // no `uiDir` override.
    if (opts.embeddedUi) {
      ...
      // Embedded source exists but has no index.html — fall through
      // to the placeholder (an empty embed is what tests look like).
    }

    // -- No uiDir / no embedded assets: placeholder page --
    // The dashboard server is up, the API works, but the SPA bundle
    // hasn't been built yet (or the developer is running from source
    // without LICH_UI_DIR set). Tell the user explicitly so an unbuilt
    // build doesn't look like a server bug.
...
/**
 * In-memory counterpart to {@link serveStatic}. Normalises the URL path
 * (root → `index.html`, strip leading slash, URL-decode), looks up the
 * asset in the source, and returns a Response with the manifest's
 * content-type. Returns null on miss — caller decides whether to fall
 * back to `index.html` (the SPA-shell route) or 404.
 *
 * No path-traversal check needed: the source is a flat string-keyed
 * map, so `../etc/passwd` either matches a literal key or misses. No
 * filesystem is reachable from here.
 */
