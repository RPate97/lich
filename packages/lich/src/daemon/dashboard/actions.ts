...
// 5min accommodates supabase-style teardowns; dashboard polls so it's not user-blocking.
...
/**
 * Run `lich <action>` in `worktreePath`. Returns `{ ok: false, ... }` on
 * subprocess failure; THROWS only when the lich binary can't be located.
 */
  ...
  return new Promise<ActionResult>((resolve) => {
    ...
    try {
      child = spawn(binaryPath, [action], {
        ...
        // detached → child is process-group leader so we can SIGKILL
        // grandchildren on timeout; otherwise shell-wrapper grandchildren
        // hold stdio open and `close` never fires.
        detached: true,
      });
    } catch (err) {
      resolve({
    ...
    const timeout = setTimeout(() => {
      timedOut = true;
      // SIGKILL the process group (-pid) so grandchildren get reaped.
      try {
...
/** Resolves LICH_BIN env var, then a sibling of process.execPath named `lich`. Throws when neither exists. */
