...
export interface RunDaemonOpts {
   ...
   * with SPA fallback to `index.html`. Takes precedence over
   * {@link embeddedUi}. Production release binaries leave this unset
   * and rely on the embedded manifest; the env-var `LICH_UI_DIR`
   * (honored by `bin/lich-daemon.ts`) is the dev/test override that
   * surfaces here.
   */
  uiDir?: string;

  /**
   * Optional in-memory SPA manifest. Production release binaries pass
   * the embedded manifest generated by `scripts/embed-ui.ts` so the
   * dashboard SPA ships baked into the binary. Forwarded verbatim to
   * `startDashboardServer`; only consulted when `uiDir` is unset.
   */
