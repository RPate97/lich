 ...
 * consulted by the proxy BEFORE the per-stack routing table. Lives as a
 * separate layer because the apex doesn't fit `parseHostname`'s
 * subdomain grammar and `RoutingTable.reload()` blows away its entries
...
export interface StaticRoutes {
  ...
  /** Canonical (lowercased, port-stripped) hostnames — used in the proxy's 404 body. */
  hosts(): string[];
}
