...
// 30000-49999: above common dev-server ports and macOS's ephemeral
// range, minimizing collisions with stack-pinned ports.
...
/** Stable per-identity port in 30000-49999. SHA-256 for avalanche so nearby paths land in different buckets. */
...
// Hop-by-hop headers per RFC 9110 § 7.6.1. `host` included so upstream
// sees its own bound address, not the friendly hostname.
...
function buildUpstreamRequest(req: Request, upstreamBase: string): Request {
  ...
  // Preserve any subpath on the base (e.g. http://localhost:9000/api).
...
function buildClientResponse(upstreamRes: Response): Response {
  ...
  for (const [name, value] of upstreamRes.headers.entries()) {
    ...
    // Bun's fetch auto-decompresses but keeps content-encoding;
    // forwarding both makes the client double-decompress. Drop
    // content-length too since decompressed length differs.
...
/** Start the reverse proxy. Binds loopback only — 0.0.0.0 would expose every stack's ports to the network. */
export async function startProxy(opts: ProxyOpts): Promise<{
  ...
  const handler = async (req: Request): Promise<Response> => {
    ...
    // Static routes consulted first: the apex `lich.localhost` doesn't
    // fit the subdomain grammar parseHostname expects.
  ...
  // with ECONNREFUSED. NOT 0.0.0.0 — would accept off-host connections.
  // EADDRINUSE on the preferred port falls back to OS-assigned and warns.
  ...
  try {
    ...
    // IPv6 disabled or otherwise unavailable — IPv4 already up, continue.
    // eslint-disable-next-line no-console
    console.warn(
  ...
  return {
    // Explicit 127.0.0.1 avoids the localhost-resolution-order issue described above.
    url: `http://127.0.0.1:${actualPort}`,
    stop,
  };
}

// Structural cast to read the routing table's private entries map for
// the 404 body. Promoting a public `keys()` for one use would noise the API.
