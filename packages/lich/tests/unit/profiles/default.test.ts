...
/**
 * Build a minimal `LichConfig` from a `profiles` map. Keeps each test fixture
 * focused on the default-flag combinations the helper inspects.
 */
function configWith(
...
describe("pickDefaultProfile (multiple defaults)", () => {
  ...
  it("lists every defaulting profile in the error (sorted)", () => {
    // Insertion order is `c, a, b` but the error must list them alphabetically.
    const result = pickDefaultProfile(
      configWith({
        c: { default: true },
        a: { default: true },
