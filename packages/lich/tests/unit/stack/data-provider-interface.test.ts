...
import type { ProcTreeResponse } from "../../../src/daemon/metrics/proc-tree.js";

describe("StackDataProvider interface", () => {
  it("declares listStacks/loadStack/tailLogs/tailAllLogs/metricsLatest/metricsStream/procTree", () => {
    const fake: StackDataProvider = {
      ...
      tailAllLogs(_stackId: string, _signal: AbortSignal): ReadableStream<Uint8Array> {
        return new ReadableStream();
      },
      ...
      async procTree(_stackId: string, _serviceName: string): Promise<ProcTreeResponse | null> { return null; },
    };
    ...
    expect(typeof fake.tailAllLogs).toBe("function");
