/**
 * Unit tests for the Task 10 supervisor extensions:
 *   - multi-port (`spec.ports`)
 *   - `runOneshot` (cmd-and-exit setup services)
 *   - `stop_cmd` (custom teardown command)
 *
 * Kept in a separate file from supervisor.test.ts so Task 9's tests stay
 * untouched — the testing standards spec is firm about not coupling new
 * features to the regression coverage of old ones.
 *
 * Each test follows the same setup pattern as supervisor.test.ts:
 *   - fresh tmpdir LICH_HOME per test
 *   - ensureStackDir to create the per-stack log directory
 *   - serviceLogPath to source the log file (proves integration with the
 *     real state-directory layout)
 *   - short commands so the suite stays fast (<1s per test)
 */
