/**
 * Unit tests for `ProcessExitWatcher` and `formatProcessExitFailure`
 * (Plan 4 Task 8).
 *
 * Each test uses a fake `OwnedHandle`-shaped object — we don't spawn a
 * real process here. The watcher only consumes `handle.exited` and the
 * `readSignal()` closure, so a Promise-shaped fake plus a function is
 * everything needed to exercise every branch. Spawning real processes
 * to test stage-categorization would be slow and add cross-platform
 * flakiness for no behavioral gain (the supervisor's own tests already
 * cover real spawn).
 */
...
/**
 * Build a minimal fake `OwnedHandle` whose `exited` resolves to the given
 * result. The other handle fields (`pid`, `stop`, etc.) are tube-test
 * stubs — `ProcessExitWatcher` doesn't read them, so they only need to
...
describe("ProcessExitWatcher.wait", () => {
  ...
  it("captures the stage label from readSignal() at the moment of exit", async () => {
    // readSignal is a closure over a mutable variable. Construct the
    // watcher while stage='during_startup', flip to 'before_ready'
    // BEFORE the exited promise resolves, then assert the watcher
    // captures 'before_ready' (not 'during_startup').
    ...
    // Flip the stage AFTER construction but BEFORE the handle exits.
    // This proves the watcher samples lazily, not at construction time.
    stage = "before_ready";
  ...
  it("handles signal kill with a non-SIGKILL signal name", async () => {
    // Coverage for the broader signal branch — not just SIGKILL.
    // OOM-killer, parent-terminate, etc. all surface as different
    // signal names; the watcher should pass them through verbatim.
...
describe("formatProcessExitFailure", () => {
  it("renders exit code and stage in a readable way", () => {
    ...
    // Assert each load-bearing piece is in the string. Avoid asserting
    // exact wording so future copy-edits don't break the test for
    // cosmetic reasons.
  ...
  it("describes each lifecycle stage with distinct wording", () => {
    // Sanity-check that the three stages produce three different
    // messages — guards against a future refactor that accidentally
