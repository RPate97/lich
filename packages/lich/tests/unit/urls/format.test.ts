/**
 * Unit tests for the shared URL-formatting helpers used by `lich up`'s
 * success summary and `lich urls` (LEV-481).
 *
 * The helpers are pure functions over snapshot data — no env, no I/O, no
 * orchestrator plumbing — so the tests build synthetic routing entries
 * and service snapshots inline and assert on the formatted strings.
 *
 * Coverage:
 *   - `formatFriendlyUrl`: produces `http://<host>.lich.localhost:<proxy>/`
 *   - `extractKey`: peels port keys out of multi-port hostnames; returns
 *     null for single-port hostnames; handles dashes in worktree names.
 *   - `buildFriendlyUrls`: maps a routing array to FormattedUrl[]
 *   - `buildRawUrls`: maps services with allocated_ports to FormattedUrl[]
 *   - `formatUrlLine`: friendly vs raw separator styles
 */
