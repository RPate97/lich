...
describe("isPortFree: dual-stack awareness", () => {
  ...
  it("skips a port that's bound only on IPv6 (::), not IPv4", async () => {
    // Bind the first port of the range on IPv6 only. The allocator must
    // skip it — handing it out would EADDRINUSE on a dual-stack consumer.
