/**
 * Tests for the per-stack `routing` field on `StackSnapshot` (LEV-403,
 * Plan 5 Task 1).
 *
 * Plan 5 introduces friendly URLs via a daemon-hosted reverse proxy. Each
 * stack's `state.json` carries a `routing` block — a list of
 * `<hostname> -> <upstream URL>` pairs the proxy joins into its routing
 * table. The dashboard reads the same field for display.
 *
 * Three invariants this file pins down:
 *
 *   1. ROUND-TRIP with routing: write a snapshot with `routing: [...]`,
 *      read it back byte-for-byte equal. `sanitizeForWrite` must NOT strip
 *      the field.
 *   2. BACK-COMPAT: pre-Plan-5 snapshots have no `routing` key at all. They
 *      must parse cleanly via `readSnapshot` and yield `routing ===
 *      undefined` (not `null`, not `[]` — the field is genuinely absent).
 *   3. EMPTY-ARRAY PRESERVATION: `routing: []` round-trips as `[]`, never
 *      collapsed to `undefined`. The two cases mean different things to
 *      the proxy: `undefined` means "this snapshot doesn't know about
 *      routing"; `[]` means "this stack has no routes right now" (e.g.
 *      just torn down by `lich down`).
 */
