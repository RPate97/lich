/**
 * Unit tests for `ready_when.capture` evaluator (Plan 4 Task 6).
 *
 * The extractor (`src/ready/capture.ts`) reads `LogTail.buffer` synchronously
 * and runs a flat `key -> regex-pattern` map against it, returning a
 * `Record` of matched values. These tests cover:
 *
 *   - full-match semantics (no group → group 0)
 *   - single-group semantics (one `(...)` → group 1)
 *   - miss → `CaptureMissError` naming the missing key
 *   - first-miss-fails semantics (per spec; we don't aggregate)
 *
 * The tests use a real {@link LogTail} seeded by writing to a tmpdir file
 * and waiting for the poll loop to ingest it. We deliberately use the real
 * LogTail (not a stub) because the buffer-accumulator behaviour is what we
 * care about — capture only works because LogTail keeps everything since
 * `start()`. A stubbed `{ buffer: "..." }` object would test the regex
 * logic but miss the integration contract.
 */
