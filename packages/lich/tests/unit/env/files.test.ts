...
describe("loadEnvFiles — single file basics", () => {
  ...
  it("does NOT strip inline `# comment` after an unquoted value", async () => {
    // Conventional dotenv behavior: inline comments are NOT recognised;
    // the `#` becomes part of the value. We test this explicitly so
    // future changes don't quietly regress it.
...
describe("loadEnvFiles — read errors other than ENOENT", () => {
  it("throws with the file path in the message for non-ENOENT read errors", async () => {
    // A directory path will produce EISDIR on readFile, which is not
    // ENOENT — so the loader should surface it (not silently skip).
